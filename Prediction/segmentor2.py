...
def cropImg(im_temp2, new_rect):
    ...
    # cv2.imwrite(f'croppedImage{len(cropped_imgs)}.png', im_temp2)
...
def img_combine(img1, img2, r1, r2, base_rect):
    ...
    img1 = skimage.util.pad(img1, ((r1[1], h - r1[3] - r1[1]), (r1[0], w - r1[2] - r1[0])), 'constant',
                            constant_values=0)
    img2 = skimage.util.pad(img2, ((r2[1], h - r2[3] - r2[1]), (r2[0], w - r2[2] - r2[0])), 'constant',
                            constant_values=0)
...
def process(image_path):
    ...
    cv2.imwrite('1_original.png', im_gray)
    # Convert to grayscale and apply Gaussian filtering
    im_gray = cv2.GaussianBlur(im_gray, (11, 11), 0)
    cv2.imwrite('2_gaussianblur.png', im_gray)
    im = 255 - im_gray
    cv2.imwrite('3_negative.png', im)
    # Threshold the image
    ret, im_th = cv2.threshold(im, 220, 255, cv2.THRESH_BINARY_INV)  # 127
    cv2.imwrite('4_threshold.png', im_th)

    im2, ctrs, hier = cv2.findContours(im_th.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    cv2.imwrite('5_findContours.png', im2)
            ...
            for j in range(i, len(bars_rect) - 1):
                ...
                if overlap(bars_rect, i, j):
                    ...
                    new_rect = (min(bars_rect[i][0], bars_rect[j][0]),
                                ...
                                max(bars_rect[i][0] + bars_rect[i][2], bars_rect[j][0] + bars_rect[j][2]) - min(
                                    bars_rect[i][0], bars_rect[j][0]),
                                max(bars_rect[i][1] + bars_rect[i][3], bars_rect[j][1] + bars_rect[j][3]) - min(
                                    bars_rect[i][1], bars_rect[j][1]))
    ...
    for i in range(len(bars_rect)):
        global cropped_imgs
        rect = bars_rect[i]
        cropImg(bars_im[i], rect)
        cv2.rectangle(im, (rect[0], rect[1]), (rect[0] + rect[2], rect[1] + rect[3]), (0, 255, 0), 3)
        im = 255 - im
    cropped_imgs = [x for _, x in sorted(zip(cropped_rects, cropped_imgs))]
    [cv2.imwrite(f'cropped_img{idx}.png', im) for idx, im in enumerate(cropped_imgs)]
    return cropped_imgs, cropped_rects.sort()
