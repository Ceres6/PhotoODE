...
def _root_removal(img, img_inv, *, debug=False):
    ...
    if debug:
        print('performing radical-radicand split')
    ...
    if len(bound_rects) > 1:
        ...
        operation = ('n',)
    # FIXME: Return grouped list
    return operation, crop_imgs
...
def xy_segmentation(img_path, *, pad=[3] * 4, debug=False):
    ...
    while continue_division and len(imgs) < 4:
        ...
        imgs.append([[], []])
        for group_index, image_group in enumerate(imgs[-2][1]):
            for idx, im in enumerate(image_group):
                if not debug:
                    pass
                else:
                    print(f'starting division of order {len(imgs) - 1} of group {group_index} of picture {idx}')
                try:
                    # Expect different returns depending on OpenCV version
                    if int(cv.__version__.split('.')[0]) < 4:
                ...
                if len(contours) > 2:
                    ...
                    # FIXME: use append instead of extend and fix access
                    imgs[-1][1].append(division_results[1])
                else:
                    if debug:
